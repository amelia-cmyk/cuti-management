...
export default function Dashboard() {
  ...
  const handleEditPegawai = (p: any) => {
    ...
    setFormPegawai({
      namaDepan: p.namaDepan || "",
      namaBelakang: p.namaBelakang || "",
      email: p.email || "",
      noHp: p.noHp || "",
      alamat: p.alamat || "",
  ...
  return (
    <main className="p-6 max-w-5xl mx-auto">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold">Dashboard Cuti Pegawai</h1>
      ...
      <div className="bg-white shadow-md rounded-xl p-4 mb-6">
        ...
        <form onSubmit={handleSubmitCuti} className="grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          >
            <option value="">Pilih Pegawai</option>
            {pegawai.map((p) => (
              <option key={p.id} value={String(p.id)}>{p.namaDepan} {p.namaBelakang}</option>
            ))}
          </select>
          <input
            type="date"
